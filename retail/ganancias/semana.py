...
class Semana(tk.Frame):
    def __init__(self, parent):
        ...
        self.tree.heading("num_semana", text="# Semana")
        ...
        self.tree.column("num_semana", width=80, anchor="center")
        ...
        self.btn_pdf = tk.Button(
            ...
            bd=0,
            cursor="hand2",
            command=self.generar_pdf_semana,
            padx=10,
            anchor="w",
        )
    ...
    def cargar_datos(self):
        ...
        while actual_inicio <= fecha_fin:
            actual_fin = actual_inicio + datetime.timedelta(days=6)
            semanas.append((num_semana, actual_inicio, actual_fin))
            actual_inicio = actual_fin + datetime.timedelta(days=1)
            num_semana += 1

        resumen_semanal = []
        for num_semana, sem_ini, sem_fin in semanas:
            total_ventas = 0
            total_ganancia = 0
            ...
            if productos_vendidos > 0:
                resumen_semanal.append(
                    (
                        num_semana,
                        sem_ini.strftime("%Y-%m-%d"),
                        sem_fin.strftime("%Y-%m-%d"),
        ...
        self.after(0, self._actualizar_tabla, resumen_semanal)

    def _actualizar_tabla(self, resumen_semanal):
